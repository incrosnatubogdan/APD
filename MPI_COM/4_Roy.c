		...
		for (int i = my_rank; i < N; i = i + nrProcesses)
		{
			for (int j = 0; j < N; j++)
			{
				if (a[i][j] > a[i][k] + a[k][j])
					a[i][j] = a[i][k] + a[k][j];    //unde am infinit imi pune o valoare noua, un drum nou calculat mai scurt
			}
		}
		//min a=result, matricea costurilor minime
		/*
		combina val expdiate de toate procesele 
		-a 
		-datele care trebuie trimise
		-numărul elementelor din bufferul de trimitere || mărimea tamponului de date
		-tipul elementelor sun de tip int
		-numarul minim
		-radacina
		-comunicatorul
		*/
		...
		if (my_rank == 0) {
			for (int i = 0; i < N; i++)
				for (int j = 0; j < N; j++)
					a[i][j] = result[i][j];
			MPI_Bcast(a, N*N, MPI_INT, 0, MPI_COMM_WORLD); //trimite mesaj cu rangul radacina la toate celelalte procese
		}
		MPI_Barrier(MPI_COMM_WORLD); //blocheaza procesul curent pana cand toate procesele ajung la comunicatorul actual
	}
